...
VeloxKVServer::VeloxKVServer(uint16_t p)
    : server_socket(-1), port(p), running(false), cache_initialized(false) {
...
std::string VeloxKVServer::handle_init(const std::vector<std::string>& args) {
    ...
    auto factory = [](uint32_t c) {
        return std::make_unique<LRUCache<std::string, std::string>>(c);
    };
    ...
    try {
        ...
        switch (cache_type) {
            case CacheType::SERIAL: type_name = "SERIAL"; break;
            case CacheType::CONCURRENT: type_name = "CONCURRENT"; break;
            case CacheType::SHARDED: type_name = "SHARDED"; break;
        }
